...
# Updated path per request
# ---------------------------------------------------------------------------------
DEFAULT_G_PATH = r"G:\02_Folder 5S\FF === Group ===\04_Powertrain\10_Test machine\Test plan database (Do not Move)\Machine booking"
...
# ---------------------------------------------------------------------------------
# Default seed data - Updated Machine List
# ---------------------------------------------------------------------------------
DEFAULT_MACHINES = [
    {"id": "wp_v",      "name": "Water pump type V"},
    {"id": "wp_iv_1",   "name": "Water pump type IV No.1"},
    {"id": "wp_iv_2",   "name": "Water pump type IV No.2"},
    {"id": "wp_iv_6",   "name": "Water pump type IV No.6"},
    {"id": "wp_iv_7",   "name": "Water pump type IV No.7"},
    {"id": "hs_mb",     "name": "High speed Medium box"},
    {"id": "mb_new",    "name": "Medium box (New)"},
    {"id": "mb_old",    "name": "Medium Box (Old)"},
    {"id": "mcb",       "name": "MCB"},
    {"id": "tp",        "name": "TP"},
]
...
@app.route("/plan-pt-api/schedule", methods=["GET"])
def get_schedule():
    try:
        ensure_seed()
        ...
        # Merge/Check for new default machines if missing from file
        existing_ids = {m["id"] for m in content.get("machines", [])}
        added = False
        for dm in DEFAULT_MACHINES:
            if dm["id"] not in existing_ids:
                content.setdefault("machines", []).append(dm)
                added = True
        
        # If we added new machines structure, save it back so it persists
        if added:
             with FileLock(LOCK_PATH):
                safe_write_json(DATA_PATH, content)
...
@app.route("/plan-pt-api/auth-check", methods=["GET"])
@require_api_key
def auth_check():
    return jsonify(ok=True)

@app.route("/plan-pt-api/schedule", methods=["POST"])
@require_api_key
def save_schedule():
    try:
...
if __name__ == "__main__":
    PORT = int(os.environ.get("PORT", "4019"))
