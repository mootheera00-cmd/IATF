...
const authRouter: any = require('./auth');
const authRequired = authRouter.authRequired;
const requireRole = authRouter.requireRole;

// Create Change Request
router.post('/', authRequired, requireRole(['CHANGE_REQUESTER', 'ADMIN']), (req: any, res: any) => {
  ...
  const sql = `
        ...
        VALUES (?, ?, ?, ?, 'CR_SUBMITTED')
    `;
  db.run(sql, [document_id, req.user.id, reason, impact], function (err: any) {
...
router.post('/:id/review', authRequired, requireRole(['REVIEWER', 'PROCESS_OWNER', 'ADMIN']), (req: any, res: any) => {
  const db = req.app.locals.db;
  const crId = req.params.id;
  db.run("UPDATE change_requests SET status='CR_REVIEWED', reviewer_id=? WHERE id=?", [req.user.id, crId], function (err: any) {
...
router.post('/:id/approve', authRequired, requireRole(['APPROVER', 'ADMIN']), (req: any, res: any) => {
  const db = req.app.locals.db;
  const crId = req.params.id;
  db.run("UPDATE change_requests SET status='CR_APPROVED', approver_id=? WHERE id=?", [req.user.id, crId], function (err: any) {
...
router.post('/:id/release', authRequired, requireRole(['DOCUMENT_CONTROL', 'ADMIN']), (req: any, res: any) => {
  const db = req.app.locals.db;
  const crId = req.params.id;
  db.run("UPDATE change_requests SET status='CR_RELEASED' WHERE id=?", [crId], function (err: any) {
