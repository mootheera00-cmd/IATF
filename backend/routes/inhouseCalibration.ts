...
const PAGE_KEY = 'inhouse_calibration'; // unique key for this page's PIC setting

function ensureTable(db: any, cb: (err?: any) => void) {
  db.run(
    `CREATE TABLE IF NOT EXISTS InHouseCalibrationEquipment (
    ...
    (err: any) => {
      ...
      newCols.forEach(([col, type]) => {
        db.run(`ALTER TABLE InHouseCalibrationEquipment ADD COLUMN ${col} ${type}`, () => {
  ...
  ensureTable(db, () => {
    ...
    db.all(
      `SELECT *,
       ...
       FROM InHouseCalibrationEquipment
  ...
  ensureTable(db, () => {
    ...
    db.get(
      `SELECT
       ...
       FROM InHouseCalibrationEquipment WHERE status != 'Retired'`,
...
router.post('/', authRequired, (req: any, res: any) => {
  ...
  ensureTable(db, () => {
    db.run(
      `INSERT INTO InHouseCalibrationEquipment
...
function doUpdate(db: any, req: any, res: any) {
  ...
  db.run(
    `UPDATE InHouseCalibrationEquipment SET
...
router.delete('/:id', authRequired, (req: any, res: any) => {
  ...
  if (!isPrivileged(userRole)) {
    return db.get(
      ...
      (err: any, row: any) => {
        ...
        db.run(`DELETE FROM InHouseCalibrationEquipment WHERE id = ?`, [req.params.id], function (this: any, err2: any) {
  ...
  db.run(`DELETE FROM InHouseCalibrationEquipment WHERE id = ?`, [req.params.id], function (this: any, err: any) {
    if (err) return res.status(500).json({ error: err.message });
    res.json({ message: 'Deleted' });
  });
});
