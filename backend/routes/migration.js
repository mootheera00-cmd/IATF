...
const logAudit = (db, userId, action, details) => {
    return new Promise((resolve, reject) => {
        ...
        db.run(createTable, [], (err) => {
            ...
            db.run(insert, [userId, action, details], (err) => {
                if (err) {
...
router.post('/migrate', authRequired, requireRole(['ADMIN']), (req, res) => {
    upload(req, res, async (err) => {
        ...
        if (!req.files || !req.files['pdf_file'] || !req.files['source_file']) {
            ...
            if (req.files?.['pdf_file']) try { fs.unlinkSync(req.files['pdf_file'][0].path); } catch(e){}
            if (req.files?.['source_file']) try { fs.unlinkSync(req.files['source_file'][0].path); } catch(e){}
            
            return res.status(400).json({ message: 'Validation Failed: Both PDF (Controlled Copy) and Master Source File are required simultaneousely.' });
        }
        ...
        const sanitizer = str => str.replace(/[^a-zA-Z0-9_\-]/g, '_');
        ...
        if (fs.existsSync(finalPdfPath) || fs.existsSync(finalSourcePath)) {
            // Clean up temp files
            try { fs.unlinkSync(pdfFile.path); fs.unlinkSync(sourceFile.path); } catch(e){}
            return res.status(409).json({ message: `Document ${doc_no} Rev ${revision} already exists. Increment revision or archive old version.` });
        }
        ...
        db.serialize(() => {
            ...
            db.get(`SELECT id FROM Document WHERE doc_number = ?`, [doc_no], (err, row) => {
                if (err) {
                ...
                const finalizeMigration = (documentId) => {
                            ...
                            (updateErr) => {
                                ...
                                logAudit(db, userId, 'MIGRATE_LEGACY', auditDetails)
                                    ...
                                    .catch((auditErr) => {
                                        ...
                                        db.run('ROLLBACK');
                                        return res.status(500).json({ message: 'Compliance Error: Audit logging failed. Transaction aborted.' });
                                    });
                            }
                        );
                    });
                ...
                db.run(insertDocSql, [doc_no, title, level, timestamp], function (insertErr) {
                    if (insertErr) {
