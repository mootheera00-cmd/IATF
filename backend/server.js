...
const ALLOWED_ORIGINS = new Set((process.env.CORS_ORIGIN || 'http://localhost:5173,http://127.0.0.1:5173,http://133.124.150.22:5173').split(',').map((o) => o.trim()));
app.use(cors({
...
app.post('/api/shared-buttons', authRequired, (req, res) => {
    ...
    req.db.run('INSERT INTO shared_buttons (label, path, created_by) VALUES (?, ?, ?)', [trimLabel, trimPath, req.user.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
...
app.get('/api/kpi-csv', authRequired, (req, res) => {
    req.db.get('SELECT file_name, csv_json FROM kpi_csv_data WHERE id = 1', [], (err, row) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (!row)
            return res.json({ fileName: '', data: null });
        try {
...
app.post('/api/kpi-csv', authRequired, (req, res) => {
    ...
    req.db.run(`INSERT INTO kpi_csv_data (id, file_name, csv_json, uploaded_by, uploaded_at)
     VALUES (1, ?, ?, ?, CURRENT_TIMESTAMP)
     ON CONFLICT(id) DO UPDATE SET file_name = excluded.file_name, csv_json = excluded.csv_json, uploaded_by = excluded.uploaded_by, uploaded_at = CURRENT_TIMESTAMP`, [name, jsonStr, req.user.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json({ success: true });
    });
