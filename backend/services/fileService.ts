...
const fileService = {
   ...
   */
  validateFileType(fileName: string, fileType: string, expectedTypes: string[] = []) {
    const ext = path.extname(fileName).toLowerCase();
    const validExtensions = expectedTypes.length > 0 ? expectedTypes : ['.doc', '.docx', '.xls', '.xlsx'];
   ...
   * Validate PDF file
   */
  validatePdfFile(fileName: string) {
    const ext = path.extname(fileName).toLowerCase();
    if (ext !== '.pdf') {
   ...
   * Check file size
   */
  validateFileSize(filePath: string, maxSizeMB = 50) {
    if (!fs.existsSync(filePath)) {
      throw new Error(`File not found: ${filePath}`);
    }
   ...
   */
  async storeOriginalFile(sourceFile: any, documentId: any, revisionId: any) {
    try {
      ...
      return new Promise((resolve, reject) => {
        fs.rename(sourceFile.path, targetPath, async (err: any) => {
          if (err) {
   ...
   * Store PDF file
   */
  async storePdfFile(pdfFile: any, documentId: any, revisionId: any) {
    try {
      ...
      // Move file to permanent storage
      return new Promise((resolve, reject) => {
        fs.rename(pdfFile.path, targetPath, async (err: any) => {
          if (err) {
   ...
   */
  async getFile(filePath: string, userRole: string | null = null) {
    try {
      if (!fs.existsSync(filePath)) {
   ...
   */
  async verifyFileIntegrity(filePath: string, expectedHash: string) {
    try {
      if (!fs.existsSync(filePath)) {
        throw new Error('File not found');
      }
