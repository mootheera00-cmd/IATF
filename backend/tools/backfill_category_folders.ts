// tools/backfill_category_folders.ts
...
function toCategoryFolder(value: any) {
  const raw = String(value || 'uncategorized').trim().toLowerCase();
  return raw.replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'uncategorized';
}

function toUnixPath(value: any) {
  return String(value || '').replace(/\\/g, '/');
}

function isAlreadyCategorized(storedPath: string, kind: 'pdf' | 'original') {
...
function resolveExistingPath(storedPath: string, kind: 'pdf' | 'original') {
...
function getRootForColumn(column: string, kind: 'pdf' | 'original') {
...
function buildTargetPath(sourceAbsolutePath: string, kind: 'pdf' | 'original', categoryFolder: string, revisionId: any, column: string) {
...
function makeStoredValue(column: string, originalValue: string, targetAbsolutePath: string, kind: 'pdf' | 'original', categoryFolder: string) {
...
async function main() {
  const docRevColumns: any[] = (await all('PRAGMA table_info(DocumentRevision)')) as any[];
  const hasColumn = (name: string) => docRevColumns.some((col: any) => col.name === name);
  ...
  for (const row of rows) {
    ...
    const fields = [
      { column: 'file_path_pdf', kind: 'pdf' as const },
      { column: 'pdf_uri', kind: 'pdf' as const },
      { column: 'file_path_original', kind: 'original' as const },
      { column: 'original_uri', kind: 'original' as const }
