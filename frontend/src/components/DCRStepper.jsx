...
export default function DCRStepper({ currentStatus, history = [] }) {
  // Define the standard workflow steps
  const steps = [
    { id: 1, label: 'Draft', statusKey: ['Draft'] },
    { id: 2, label: 'DC Initial', statusKey: ['Submitted', 'DC Approved'] },
    { id: 3, label: 'Checker', statusKey: ['Pending Checker Approval', 'Pending Approval', 'Pending Approver Approval'] },
    { id: 4, label: 'Approver', statusKey: ['Pending DC Final Approval'] },
    { id: 5, label: 'Released', statusKey: ['Released', 'Approved'] }, 
  ];
  ...
  const getStepStatus = (step, index) => {
    ...
    // If status not found/matched (e.g. Rejected), handle separately or default
    if (currentStatus === 'Rejected' || currentStatus === 'Returned for Revision') return 'error'; 

    if (currentStepIndex === -1) {
       // Maybe 'Revision Pending' goes back to Draft or Dept Review? 
       // For now, let's assume if index < currentStepIndex it's completed
       return 'pending';
    }
  ...
  // Helper to get audit info for a step
  const getStepAudit = (step) => {
    // This is a heuristic mapping since history might be just a list of logs
    // We try to find a log entry that corresponds to moving TO this step or completing it.
    // For simplicity, we just look for approvals in history.
    // In a real app, history would have 'step' or 'stage' metadata.
